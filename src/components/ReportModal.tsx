    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-slate-300">
          ...
          {activeTab === 'legal' && (
            <div className="space-y-6">
              {/* Judicial Admissibility Declaration or Degraded Fallback Banner */}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-amber-200">
                        DEGRADED MODE — UNVERIFIED CLIENT HEURISTIC (NON-EVIDENTIARY)
                      </h3>
                      <p className="text-xs text-amber-300/80 mt-1 leading-relaxed">
                        This record was parsed via a client-side offline fallback because the backend analysis pipeline was unavailable. It has <strong>not undergone multi-layer server verification, authenticated reverse-DNS, or ML classifier scoring</strong>. This output is <strong>not certified under FRE 902(11)/902(14) or ISO/IEC 27037:2012</strong> and is not suitable for formal legal or law-enforcement referral without full backend re-ingestion.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-slate-400">
                  ...
                  {analysis.analysisSource === 'client_fallback_unverified' || analysis.isClientFallback ? (
                    <span className="text-amber-400 flex items-center gap-1 font-bold">
                      <AlertTriangle className="w-4 h-4" /> CLIENT LOCAL DIGEST (UNVERIFIED PIPELINE)
                    </span>
                  ) : (
